...
async def federalrunner_execute_wizard(
    ...
    4. P Map user_data (field_id) � field_values (selector) � THE CRITICAL MAPPING
    ...
    try:
        ...
        logger.info(f"=� Executing wizard: {wizard_id}")
        ...
        # 4. P MAP user_data (field_id) � field_values (selector)
        ...
        for page in wizard.pages:
            for field in page.fields:
                ...
                if field_id in user_data:
                    ...
                    logger.debug(f"  Mapped: {field_id} � {field.selector} = {user_data[field_id]}")

        logger.info(f" Mapped {len(field_values)} fields (field_id � selector)")
