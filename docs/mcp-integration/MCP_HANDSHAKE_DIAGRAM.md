# MCP Handshake: Complete OAuth 2.1 + MCP Protocol Flow

This document provides the complete technical sequence diagram for the MCP server integration with Claude.ai/Android using OAuth 2.1 authentication and Dynamic Client Registration.

## Overview

Here's the simplified 4-step flow:

![OAuth 2.1 + MCP Handshake Simplified](../images/oauth2.1-mcp-handshake-simplified.png)

**Quick Summary:**
1. **Discover OAuth** - Claude fetches `/.well-known/oauth-protected-resource`
2. **Register Client** - Dynamic Client Registration (DCR) with Auth0
3. **Issue Token** - User authenticates and grants permissions
4. **Use Tools** - Token validated, 4 MDCalc tools available

## Detailed 24-Step Sequence

The complete technical handshake broken down into 4 phases:

```
┌─────────────┐                    ┌─────────────┐                    ┌─────────────┐
│  Claude.ai  │                    │ MCP Server  │                    │   Auth0     │
│  (Client)   │                    │ (Cloud Run) │                    │ (Auth Srv)  │
└──────┬──────┘                    └──────┬──────┘                    └──────┬──────┘
       │                                  │                                   │
       │                                  │                                   │
       ├─────────────── PHASE 1: DISCOVERY ──────────────┤                   │
       │                                  │                                   │
       │ 1. HEAD /                        │                                   │
       │  (Protocol check)                │                                   │
       │─────────────────────────────────>│                                   │
       │                                  │                                   │
       │ 2. 200 OK                        │                                   │
       │  MCP-Protocol-Version: 2025-06-18│                                   │
       │<─────────────────────────────────│                                   │
       │                                  │                                   │
       │ 3. GET /.well-known/oauth-       │                                   │
       │    protected-resource            │                                   │
       │  (Discover OAuth config)         │                                   │
       │─────────────────────────────────>│                                   │
       │                                  │                                   │
       │ 4. 200 OK                        │                                   │
       │  {                               │                                   │
       │    "authorization_servers": [    │                                   │
       │      "https://dev-ue0m6l3w4kp2..." │                                 │
       │    ],                            │                                   │
       │    "scopes_supported": [...]     │                                   │
       │  }                               │                                   │
       │<─────────────────────────────────│                                   │
       │                                  │                                   │
       │                                  │                                   │
       ├────────────── PHASE 2: OAUTH FLOW ──────────────────────────────────┤
       │                                  │                                   │
       │ 5. POST /oidc/register           │                                   │
       │  (Dynamic Client Registration)   │                                   │
       │  {                               │                                   │
       │    "client_name": "Claude MCP",  │                                   │
       │    "redirect_uris": [...],       │                                   │
       │    "grant_types": ["..."],       │                                   │
       │    "token_endpoint_auth_method": │                                   │
       │      "none"                      │                                   │
       │  }                               │                                   │
       │──────────────────────────────────────────────────────────────────────>│
       │                                  │                                   │
       │ 6. 201 Created                   │                                   │
       │  {                               │                                   │
       │    "client_id": "auto-generated" │                                   │
       │  }                               │                                   │
       │<──────────────────────────────────────────────────────────────────────│
       │                                  │                                   │
       │ 7. GET /authorize                │                                   │
       │  (Authorization Request + PKCE)  │                                   │
       │  ?client_id=...                  │                                   │
       │  &redirect_uri=...               │                                   │
       │  &response_type=code             │                                   │
       │  &scope=mdcalc:read+...          │                                   │
       │  &code_challenge=sha256(...)     │                                   │
       │  &code_challenge_method=S256     │                                   │
       │──────────────────────────────────────────────────────────────────────>│
       │                                  │                                   │
       │         [User sees Auth0 login page]                                 │
       │         [User enters email/password]                                 │
       │         [User clicks "Agree" on consent screen]                      │
       │                                  │                                   │
       │ 8. 302 Redirect                  │                                   │
       │  Location: https://claude.ai/    │                                   │
       │    oauth/callback                │                                   │
       │    ?code=authorization_code      │                                   │
       │    &state=...                    │                                   │
       │<──────────────────────────────────────────────────────────────────────│
       │                                  │                                   │
       │ 9. POST /oauth/token             │                                   │
       │  (Token Exchange)                │                                   │
       │  {                               │                                   │
       │    "grant_type":                 │                                   │
       │      "authorization_code",       │                                   │
       │    "code": "authorization_code", │                                   │
       │    "redirect_uri": "...",        │                                   │
       │    "client_id": "...",           │                                   │
       │    "code_verifier": "..."        │                                   │
       │  }                               │                                   │
       │──────────────────────────────────────────────────────────────────────>│
       │                                  │                                   │
       │ 10. 200 OK                       │                                   │
       │  {                               │                                   │
       │    "access_token":               │                                   │
       │      "eyJhbGci...JWE_TOKEN",     │                                   │
       │    "token_type": "Bearer",       │                                   │
       │    "expires_in": 86400,          │                                   │
       │    "scope": "mdcalc:read ..."    │                                   │
       │  }                               │                                   │
       │<──────────────────────────────────────────────────────────────────────│
       │                                  │                                   │
       │                                  │                                   │
       ├────────────── PHASE 3: MCP HANDSHAKE ───────────┤                   │
       │                                  │                                   │
       │ 11. POST / initialize            │                                   │
       │  Authorization: Bearer eyJhbGci..│                                   │
       │  (Token present but NOT VALIDATED)                                   │
       │  {                               │                                   │
       │    "capabilities": {},           │                                   │
       │    "name": "claude-ai",          │                                   │
       │    "version": "0.1.0"            │                                   │
       │  }                               │                                   │
       │─────────────────────────────────>│                                   │
       │                                  │                                   │
       │              ⚠️  NO TOKEN VALIDATION! ⚠️                             │
       │              ⚠️  NO Auth0 call! ⚠️                                   │
       │                                  │                                   │
       │ 12. 200 OK                       │                                   │
       │  MCP-Session-ID: ed0fee05-...    │                                   │
       │  {                               │                                   │
       │    "protocolVersion":            │                                   │
       │      "2025-06-18",               │                                   │
       │    "capabilities":               │                                   │
       │      {"tools": {}},              │                                   │
       │    "serverInfo": {...}           │                                   │
       │  }                               │                                   │
       │<─────────────────────────────────│                                   │
       │                                  │                                   │
       │ 13. POST /                       │                                   │
       │     notifications/initialized    │                                   │
       │  Authorization: Bearer eyJhbGci..│                                   │
       │  MCP-Session-ID: ed0fee05-...    │                                   │
       │  (Token present but NOT VALIDATED)                                   │
       │  {                               │                                   │
       │    "method":                     │                                   │
       │      "notifications/initialized" │                                   │
       │  }                               │                                   │
       │─────────────────────────────────>│                                   │
       │                                  │                                   │
       │              ⚠️  SESSION VALIDATION ONLY! ⚠️                         │
       │              ⚠️  NO OAuth validation! ⚠️                             │
       │              ⚠️  NO Auth0 call! ⚠️                                   │
       │                                  │                                   │
       │ 14. 202 Accepted                 │                                   │
       │  MCP-Session-ID: ed0fee05-...    │                                   │
       │<─────────────────────────────────│                                   │
       │                                  │                                   │
       │ 15. GET /                        │                                   │
       │  (Test for SSE support)          │                                   │
       │─────────────────────────────────>│                                   │
       │                                  │                                   │
       │ 16. 405 Method Not Allowed       │                                   │
       │  Allow: POST, HEAD, DELETE       │                                   │
       │  (Signals: "POST-only server")   │                                   │
       │<─────────────────────────────────│                                   │
       │                                  │                                   │
       │                                  │                                   │
       ├──────────── PHASE 4: TOOL OPERATIONS ───────────┼───────────────────┤
       │                                  │                                   │
       │ 17. POST / tools/list            │                                   │
       │  Authorization: Bearer eyJhbGci..│                                   │
       │  MCP-Session-ID: ed0fee05-...    │                                   │
       │  (FIRST TOKEN VALIDATION!)       │                                   │
       │─────────────────────────────────>│                                   │
       │                                  │                                   │
       │                                  │ 18. POST /userinfo                │
       │                                  │  Authorization:                   │
       │                                  │    Bearer eyJhbGci...             │
       │                                  │  (Validate JWE token)             │
       │                                  │──────────────────────────────────>│
       │                                  │                                   │
       │                                  │ 19. 200 OK                        │
       │                                  │  {                                │
       │                                  │    "sub":                         │
       │                                  │      "auth0|68e19346...",         │
       │                                  │    "email":                       │
       │                                  │      "user@example.com"           │
       │                                  │  }                                │
       │                                  │  ✅ Token valid!                  │
       │                                  │<──────────────────────────────────│
       │                                  │                                   │
       │ 20. 200 OK                       │                                   │
       │  {                               │                                   │
       │    "tools": [                    │                                   │
       │      {"name":                    │                                   │
       │        "mdcalc_list_all", ...},  │                                   │
       │      {"name":                    │                                   │
       │        "mdcalc_search", ...},    │                                   │
       │      {"name":                    │                                   │
       │        "mdcalc_get_calculator",..│                                   │
       │      {"name":                    │                                   │
       │        "mdcalc_execute", ...}    │                                   │
       │    ]                             │                                   │
       │  }                               │                                   │
       │<─────────────────────────────────│                                   │
       │                                  │                                   │
       │ 21. POST / tools/call            │                                   │
       │  {                               │                                   │
       │    "method": "tools/call",       │                                   │
       │    "params": {                   │                                   │
       │      "name": "mdcalc_search",    │                                   │
       │      "arguments": {              │                                   │
       │        "query": "HEART score"    │                                   │
       │      }                           │                                   │
       │    }                             │                                   │
       │  }                               │                                   │
       │─────────────────────────────────>│                                   │
       │                                  │                                   │
       │                                  │ 22. POST /userinfo                │
       │                                  │  Authorization:                   │
       │                                  │    Bearer eyJhbGci...             │
       │                                  │  (Validate token again)           │
       │                                  │──────────────────────────────────>│
       │                                  │                                   │
       │                                  │ 23. 200 OK                        │
       │                                  │  ✅ Token still valid!            │
       │                                  │<──────────────────────────────────│
       │                                  │                                   │
       │ 24. 200 OK                       │                                   │
       │  {                               │                                   │
       │    "content": [                  │                                   │
       │      {                           │                                   │
       │        "type": "text",           │                                   │
       │        "text":                   │                                   │
       │          "{...search results...}"│                                   │
       │      }                           │                                   │
       │    ]                             │                                   │
       │  }                               │                                   │
       │<─────────────────────────────────│                                   │
       │                                  │                                   │
       │ [Session continues...]           │                                   │
       │                                  │                                   │
```

---

## The Three Components Clearly Defined

### 1. **Claude.ai (Client)**
- **Location**: Anthropic's infrastructure (cloud-hosted)
- **Role**: MCP client that initiates all requests
- **User-facing**: This is what the user interacts with
- **User-Agent in logs**: `python-httpx/0.27.0` (discovery), then `Claude-User` (operations)

### 2. **MCP Server (Your Service on Google Cloud Run)**
- **Location**: `https://mdcalc-mcp-server-kxncltpzpa-uc.a.run.app`
- **Role**: Implements MCP protocol and provides MDCalc tools
- **What it does**: 
  - Responds to protocol checks
  - Advertises OAuth configuration
  - Manages MCP sessions
  - Validates tokens with Auth0
  - Executes MDCalc calculators

### 3. **Auth0 (Authorization Server)**
- **Location**: `https://dev-ue0m6l3w4kp22sn5.us.auth0.com`
- **Role**: OAuth 2.1 authorization server
- **What it does**:
  - Handles Dynamic Client Registration
  - Authenticates users (login page)
  - Issues access tokens
  - Validates tokens via /userinfo endpoint

---

## Communication Patterns

### **Phase 1 (Steps 1-4): Claude ↔ MCP Server**
```
Claude.ai ←──────────→ MCP Server (Cloud Run)
                       
                       Auth0 (not involved)
```
**Purpose**: Protocol discovery

---

### **Phase 2 (Steps 5-10): Claude ↔ Auth0**
```
Claude.ai ←──────────────────────────────→ Auth0
           
           MCP Server (not involved)
```
**Purpose**: User authentication and token acquisition

---

### **Phase 3 (Steps 11-16): Claude ↔ MCP Server**
```
Claude.ai ←──────────→ MCP Server (Cloud Run)
                       
                       Auth0 (not contacted!)
```
**Purpose**: MCP handshake (deliberately without token validation)

---

### **Phase 4 (Steps 17-24): Three-Way Communication**
```
Claude.ai ←──────────→ MCP Server (Cloud Run) ←──────────→ Auth0
```
**Purpose**: Tool operations with full authentication

---

## Visual Summary of Who Talks to Whom

```
╔════════════════════════════════════════════════════════════════╗
║  COMMUNICATION MATRIX                                          ║
╠════════════════════════════════════════════════════════════════╣
║                                                                ║
║  Phase 1 - Discovery:                                          ║
║  ┌──────────┐                    ┌──────────┐                 ║
║  │ Claude   │ ←──────────────→   │   MCP    │                 ║
║  └──────────┘                    └──────────┘                 ║
║                                                                ║
║  Phase 2 - OAuth:                                              ║
║  ┌──────────┐                                  ┌──────────┐   ║
║  │ Claude   │ ←────────────────────────────→   │  Auth0   │   ║
║  └──────────┘                                  └──────────┘   ║
║                                                                ║
║  Phase 3 - Handshake:                                          ║
║  ┌──────────┐                    ┌──────────┐                 ║
║  │ Claude   │ ←──────────────→   │   MCP    │                 ║
║  └──────────┘                    └──────────┘                 ║
║                                                                ║
║  Phase 4 - Operations:                                         ║
║  ┌──────────┐     ┌──────────┐     ┌──────────┐              ║
║  │ Claude   │ ←→  │   MCP    │ ←→  │  Auth0   │              ║
║  └──────────┘     └──────────┘     └──────────┘              ║
║                                                                ║
╚════════════════════════════════════════════════════════════════╝
```

Now all three components are clearly separated and their interactions are properly shown! 🎯